...
impl WebRTCStream {
    ...
    fn get_ice_server_from_url(url: &str) -> Option<RTCIceServer> {
        // Standard TURN URLs may include username and credential before the host.
        match Url::parse(url) {
            Ok(u) => {
                if u.scheme() == "turn"
                    || u.scheme() == "turns"
...
mod tests {
    ...
    fn test_webrtc_ice_url() {
        ...
        let turn_user_url = concat!("turn://123@", "example.com");
        assert_eq!(
            WebRTCStream::get_ice_server_from_url(turn_user_url)
                .unwrap_or_default()
                .username,
            "123"
        );
        ...
        let turn_user_auth_url = concat!("turn://123:", "321@", "example.com");
        assert_eq!(
            WebRTCStream::get_ice_server_from_url(turn_user_auth_url)
                .unwrap_or_default()
                .credential,
            "321"
        );
        ...
        let unsupported_auth_url = concat!("http://123:", "123@", "example.com:3478");
        assert_eq!(
            WebRTCStream::get_ice_server_from_url(unsupported_auth_url),
            None
        );
